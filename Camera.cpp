...
void Camera::updateMatrix(float FOVdeg, float nearPlane, float farPlane)
{
	...
	projection = glm::perspective(glm::radians(FOVdeg), (float)width / height, nearPlane, farPlane) * unitsScale;
...
void Camera::Inputs(GLFWwindow* window , float deltaTime)
{
	...
	if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS)
	{
		...
		if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS)
		{
			...
			Position += rotX * -Up * speed * 10.0f * deltaTime;
			Position += rotY * glm::normalize(glm::cross(Orientation, Up)) * speed * 10.0f * deltaTime;
